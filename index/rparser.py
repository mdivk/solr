...
class XMLToJson():
    def __init__(self):
        ...
        Author: Raymond Xie
        Date: 20180608
        Function: to extract meta data from compressed bz2 files from WORM drive and save to JSON format in the new location
        Usage: python2.7 rparser.py -c conf\conf.parser
        Sample:
        -c conf\conf.parser
        msgtype=fix
        region=apac
        flow=apeq
        path=odac\apac\apeq\20180401
        output=json\apac\apeq\20180401
        '''

        self.json_data = []
        self.process_options(conf)
        #Original design is to save json to same hierachy as input
        #output_f = os.path.join(self.options.output_dir, self.options.input_path.strip('\\'))
        output_f = self.options.output_dir
        self.create_log_directory()
        self.config_logger()
        self.create_path_if_not_exists(output_f)
        self.logger_location = ''

    def process_options(self, the_conf):
        '''
        ...
        parser = OptionParser()
        parser.add_option("-c", "--conf", dest="conf", help="Enter conf location", metavar="PATH")
        ...
        if not options.conf:
            if not the_conf:
                parser.error('conf not provided')
        else:
            ...
            msgtype = conf[0].split('=')[1].rstrip()
            region = conf[1].split('=')[1].rstrip()
            flow = conf[2].split('=')[1].rstrip()
            path = conf[3].split('=')[1].rstrip()
            output = conf[4].split('=')[1].rstrip()
    ...
    def config_logger(self):
        ...
        self.logger = logging.getLogger('msg2json_log')
    ...
    def run(self):
        ...
        self.logger.info('Data Type processed in this session: ' + msgtype)
