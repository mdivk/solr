...
class XMLToJson():
    ...
    def config_logger(self):
        logging.basicConfig(filename = datetime.datetime.now().strftime("%Y%m%d%I%M%S") + '.log', format = '%(asctime)s %(levelname)s %(message)s')
    ...
    def process_file(self, f):
        ...
        file_data = re.sub(r'<citiml:citimlTradeNotification\s+xmlns', '<citiml:citimlTradeNotification xmlns', file_data)

        if file_data.count(delimiter1) == 0 and file_data.count('<citiml:citimlTradeNotification xmlns') > 1:
            file_data = re.sub(r'<citiml:citimlTradeNotification xmlns', '<?xml version="1.0" encoding="UTF-8"?><citiml:citimlTradeNotification xmlns', file_data)
    ...
    def run(self):
        ...
        for i, input_f in enumerate(self.get_bz2_files()):
            ...
            sys.stdout.flush()
            output_f = os.path.join(self.options.output_dir, input_f.replace(self.options.input_path, '').strip('\\').strip('/'))
            # output_f = os.path.join(self.options.output_dir, input_f.replace(os.path.dirname(self.options.input_path), '').strip('\\'))
            #output_f = os.path.join(self.options.output_dir + input_f.replace(os.path.dirname(input_f), '')).strip('/')
