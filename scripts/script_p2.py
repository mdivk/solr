        ...
        if each_data[0]:
            ...
            try:
                ...
                for line, message in enumerate(d, start=1):
                    ...
                    filename = options.bz2_archive + options.json + '\%s_%s' % (each_data[1].rsplit(".", 1)[0], line) if len(d) > 1 else \
                    each_data[1].rsplit(".", 1)[0]
                    output =  options.bz2_archive + '\\' + filename

                    with open('%s.json' % output, 'w') as json_data:
                        json.dump(
                            # new_list_data,
                            new_dict_data,
                            json_data
